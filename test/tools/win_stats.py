...
def get_stats(m: np.int64, n: np.int64, k: np.int64, iterations: np.int64,
    ...
    out = subprocess.Popen(['perf', 'stat',
                            ...
                            '-e minor-faults',
                            '-e major-faults',
                            'build/win_perf_marlin', m, n, k, iterations, mode, sparsity],
...
def iterate_over_matrix_sizes(mat_m_start: np.int64, mat_m_end: np.int64,
                              ...
                              mat_n_inc: np.int64,  mat_k_start: np.int64, mat_k_end: np.int64,
                              mat_k_inc: np.int64,
                              sparsity: np.int64,
                              iterations: np.int64):
    ...
    for kk, mat_n in enumerate(range(mat_n_start, mat_n_end, mat_n_inc)):
        for i, mat_m in enumerate(range(mat_m_start, mat_m_end, mat_m_inc)):
            for k_, mat_k in enumerate(range(mat_k_start, mat_k_end, mat_k_inc)):
                m = mat_m
                n = mat_n
                k = mat_k
                index = kk * num_m_mat * num_k_mat * num_modes + \
                    i * num_k_mat * num_modes + k_ * num_modes
                for j, mode in enumerate(range(num_modes)):
                    stats = get_stats(m, n, k, iterations, mode, sparsity)
                    results[index + j][3:3 + stat_count] = stats
                    results[index + j][0] = m
                    results[index + j][1] = n
                    results[index + j][2] = k
...
def main():
    iterate_over_matrix_sizes(1, 2, 1, 16, 80, 16, 8, 72, 8, 0, 1000)
